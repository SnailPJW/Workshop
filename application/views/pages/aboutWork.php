    <!-- 主要容器 -->
    <div class="ts very narrow container">
        <!-- 主要信件卡片 -->
        <div class="ts card">
            <!-- 頂部內容與標題 -->
            <div class="center aligned padded content">
                <div class="ts large header">
                    快來試試 沃課SHOP
                    <div class="smaller sub header">
                        這是一個以問題為導向的募師、募課及募問題的教育系統！
                    </div>
                </div>
            </div>
            <!-- / 頂部內容與標題 -->

            <!-- 特色圖片 -->
            <div class="image">
                <img class="ts large circular centered image" src="<?php echo base_url();?>asset/img/rsz_worklogo.jpg">
            </div>
            <!-- / 特色圖片 -->

            <!-- 主要推銷內容 -->
            <div class="center aligned padded content">
                <p>全世界不到 0.01% 的人都在使用最好用的 沃課SHOP！</p>
                <p>如果 IE 都有勇氣問你是否要成為預設瀏覽器了，你又何嘗沒有勇氣嘗試 沃課SHOP 呢？</p>
                <br>
                <!-- CTA：矚目動作按鈕 -->
                <button class="ts primary button" onclick="location.href='<?php echo base_url();?>index.php/pages/view/login'">讓我現在加入</button>
                <!-- / CTA：矚目動作按鈕 -->

                <!-- 區段分隔線 -->
                <div class="ts section divider"></div>
                <!-- / 區段分隔線 -->

                <!-- 特色項目群組 -->
                <div class="ts relaxed items">
                    <!-- 單個項目 -->
                    <div class="item">
                        <div class="image">
                            <img src="<?php echo base_url();?>asset/img/molecule.svg">
                        </div>
                        <div class="left aligned content">
                            <a class="header">豐富的平台</a>
                            <div class="description">
                                導入「駐校企業家/藝術家」及「商家導師」，引進商家業師的互惠教學。
                            </div>
                        </div>
                    </div>
                    <!-- / 單個項目 -->

                    <!-- 單個項目 -->
                    <div class="item">
                        <div class="image">
                            <img src="<?php echo base_url();?>asset/img/biology.svg">
                        </div>
                        <div class="left aligned content">
                            <a class="header">問題導向學習</a>
                            <div class="description">
                                針對學生的問題所在及渴望學習的內涵去設計課程，建立一個尋找問題的平台來「募問題」，學習者可以將他們的學習所需反應在平台上。
                            </div>
                        </div>
                    </div>
                    <!-- / 單個項目 -->
                </div>
                <!-- / 特色項目群組 -->

                <!-- 區段分隔線 -->
                <div class="ts section divider"></div>
                <!-- / 區段分隔線 -->

                <p>有什麼問題嗎？在這聯繫我們！</p>
                <a href="#!">support@workshop.ntut.edu.tw</a>
            </div>
            <!-- / 主要推銷內容 -->
        </div>
        <!-- / 主要信件卡片 -->

        <!-- 底部相關連結片段 -->
        <div class="ts center aligned basic segment">
            <a href="https://www.facebook.com/%E8%87%BA%E5%8C%97%E7%A7%91%E5%A4%A7%E5%85%89%E5%A4%A7%E5%89%B5%E5%89%B5%E5%AD%B8%E9%99%A2-302519096751949/" target="_blank"><i class="facebook circular large icon"></i></a>
            <a href="http://rnd.ntut.edu.tw/bin/home.php" target="_blank"><i class="sun circular large icon"></i></a>
            <a href="http://www.ntut.edu.tw/bin/home.php" target="_blank"><i class="student circular large icon"></i></a>
            <a href="https://www.slideshare.net/university2025/ss-66534774" target="_blank"><i class="slideshare circular large icon"></i></a>
        </div>
        <!-- / 底部相關連結片段 -->

        <!-- 額外附註 -->
        <div class="ts center aligned basic small message">
            <p>網站使用的圖檔聲明。</p>
            <div>Icons made by <a href="http://www.freepik.com" title="Freepik">Freepik</a> from <a href="http://www.flaticon.com" title="Flaticon">www.flaticon.com</a> is licensed by <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0" target="_blank">CC 3.0 BY</a></div>
            <div>Icons made by <a href="http://www.flaticon.com/authors/popcorns-arts" title="Icon Pond">Icon Pond</a> from <a href="http://www.flaticon.com" title="Flaticon">www.flaticon.com</a> is licensed by <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0" target="_blank">CC 3.0 BY</a></div>
        </div>
        <!-- / 額外附註 -->
    </div>
    <!-- / 主要容器 -->